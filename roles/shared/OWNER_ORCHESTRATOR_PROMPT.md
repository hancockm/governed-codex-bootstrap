...
Compose this shared Sol base with the referenced owner profile and exactly one
approved task packet. Classify risk deterministically and preserve every
...
approved behavior and paths. Use Low only when you supply the final exact
...
it is not bounded, stop Low work and return to the existing Primary task. Bind
Luna to the accepted Low candidate only when a Low receipt exists. Otherwise
bind Luna to the Primary candidate. Archive, acknowledgment, delivery, and
