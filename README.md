...
Start with the [System User Guide](docs/SYSTEM_USER_GUIDE.md) for the complete
Codex-app, vault, orchestration, continuity, closeout, and rehydration
workflow. Then use the [practical vault guide](Project_Obsidian_Vault/README.md),
open the [Project MOC](Project_Obsidian_Vault/00_Home/Project%20MOC.md), and
follow its research-first links. Shared owner instructions begin in the
[coordination instructions](Project_Obsidian_Vault/40_Coordination/Instructions/README.md).
Core vault cleanup follows the
[Core Vault Maintenance Protocol](docs/CORE_VAULT_MAINTENANCE_PROTOCOL.md).

Optional external A2A critique is configured from [.env.example](.env.example). Copy it to
...
the [External Critique Handoff](Project_Obsidian_Vault/40_Coordination/Instructions/External%20Critique%20Handoff.md).
...
| [configs/documentation_system_v1.json](configs/documentation_system_v1.json), [docs/DOCUMENTATION_SYSTEM_PARITY.md](docs/DOCUMENTATION_SYSTEM_PARITY.md) | Enforce substantive project-neutral equivalents for the instruction, Core bootstrap, continuity, Git, vault, and owner-workflow system. |
...
| --- | --- | --- |
| Authority | Who may decide and change what | [AGENTS.md](AGENTS.md), [roles/](roles), owner registry |
| Canonical knowledge | Accepted, current project truth | [Project_Obsidian_Vault/00_Canonical/](Project_Obsidian_Vault/00_Canonical) and capability registry |
| Coordination | Requests, critique, work selection, handoffs | [coordination/](coordination) and work packets |
| Execution | Bounded implementation and verification | [tools/](tools), [tests/](tests), test policy |
| Delivery | Reviewable commits and safe integration | Git policy and reconciliation tool |
| Continuity | Bounded, attributable learning history | [continuity/](continuity) and transcript exporter |
...
2. Copy each `.md`, `.txt`, or `.pdf` research file into [research/inbox/](research/inbox) and register it with `python tools/research_intake.py research/inbox/your-file.pdf --title "Short title" --origin "where it came from"`. Intake preserves exact bytes for all three formats. For a public Git repository, use `python tools/research_git_adapter.py --url "https://github.com/owner/repository.git" --ref "refs/heads/main" --commit "<full-commit>" --title "Repository research" --authorize-network`. The adapter verifies the ref against that exact commit and captures only bounded Markdown, text, and PDF blobs without checkout or execution.
...
| --- | --- |
| [Core Thesis.md](Project_Obsidian_Vault/00_Canonical/Core%20Thesis.md) | Why the project exists and accepted governing claims |
| [ARCHITECTURE.md](Project_Obsidian_Vault/00_Canonical/ARCHITECTURE.md) | Stable components, boundaries, and six-plane relationships |
| [SPEC.md](Project_Obsidian_Vault/00_Canonical/SPEC.md) | Observable behavior and non-negotiable contracts |
| [ROADMAP.md](Project_Obsidian_Vault/00_Canonical/ROADMAP.md) | Ordered, authorized capability gates |
| [Capability Registry.md](Project_Obsidian_Vault/00_Canonical/Capability%20Registry.md) | Machine-readable owner, state, evidence, and verification link |
...
bundles. [tools/origin_reconciler.py](tools/origin_reconciler.py) reports bounded reconciliation facts and,
...
[docs/TOOL_PARITY.md](docs/TOOL_PARITY.md) for the distinction between complete generic equivalents,
...
[docs/DOCUMENTATION_SYSTEM_PARITY.md](docs/DOCUMENTATION_SYSTEM_PARITY.md); conformance rejects missing operational
