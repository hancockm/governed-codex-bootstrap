...
| [governance_toolkit.py](governance_toolkit.py) | Validates the public agent-governance bundle and exact development-only tool records without network access. |
...
| [owner_scoped_orchestration.py](owner_scoped_orchestration.py) | Validates permanent path ownership, risk classifications, immutable work packets, saved-project runner-channel bindings, lane receipts, archive acknowledgments, terminal reconciliation/synchronization/worktree evidence, and immutable closeout finalization. It records but does not perform host task archival, invoke models, or mutate Git. |
...
| [role_validation.py](role_validation.py) | Validates self-hashing role requests, passed receipts, and joined candidate evidence against local ownership and role policy. |
