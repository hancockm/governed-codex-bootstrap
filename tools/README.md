...
| `research_git_adapter.py` | Captures a credential-free public HTTPS Git branch or tag at a caller-supplied full expected commit. It uses a temporary bare repository, never checks out or executes source code, records commit/tree/blob lineage, and publishes only bounded `.md`, `.txt`, and `.pdf` blobs after explicit network authorization. |
